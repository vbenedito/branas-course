...
describe("signup", () => {
  ...
  test("should not create a new passenger account with invalid name", async () => {
    ...
    expect(responseSignup.data).toEqual({ message: -3 });
  });
  ...
  test("should not create a new passenger account with invalid email", async () => {
    ...
    expect(responseSignup.status).toBe(422);
    expect(responseSignup.data).toEqual({ message: -2 });
  });
  ...
  test("should not create a new passenger account with invalid CPF", async () => {
    ...
    expect(responseSignup.status).toBe(422);
    expect(responseSignup.data).toEqual({ message: -1 });
  });
  ...
  test("should not create a new passenger when user already exists", async () => {
    ...
    expect(responseSignup.status).toBe(422);
    expect(responseSignup.data).toEqual({ message: -4 });
  });
  ...
  test("should not create a new driver account with invalid carplate", async () => {
    const input = {
      name: "john Doe",
      email: `john.doe${Math.random()}@gmail.com`,
      cpf: "41121955088",
    ...
    expect(responseSignup.status).toBe(422);
    expect(responseSignup.data).toEqual({ message: -5 });
  });
});
